...
namespace exttype{
	...
	*/
	...
	/*!
		requires: base::size(), base::operator[], base::telement;
	*/
	...
	public:
		...
		tvector operator /(const field k)const{return cpy()/=k;};
		tvector & neg(){
			for(int i=0;i<count();i++)(*ths())[i]=-(*ths())[i];
			return *ths();
		};
		...
		std::pair<field,int> min()const{
			if(count()==0)throw debug_exception("Empty vector, min is undefined");
		...
		std::pair<field,int> max()const{
			if(count()==0)throw debug_exception("Empty vector, max is undefined");
			array_entry<field> r((*this)[0],0);
			for(int i=1;i<count();i++){
				if((*this)[i]>r.value){
					r.value = (*this)[i];
		...
		std::pair<field,int> maxabs()const{
			if(count()==0)throw debug_exception("Empty vector, max is undefined");
			array_entry<field> r(std::abs((*this)[0]),0);
			for(int i=1;i<count();i++){
				field v = std::abs((*this)[i]);
				if(v>r.value){
					r.value = v;
					r.arg = i;
				};
