...
namespace exttype{
	...
	public:
		...
		fixedlist(const type & v0){
			for(int i=0;i<size();++i)(*this)[i] = v0;
		};
	...
	public:
		static const int count(int i=0){return i==0?c0:c1;};
		int2 size()const{return in2(c0,c1);};
	public:
		titerator getiterator()const{
			return tindex(int2(count(0),count(1)));
		};
	...
	public:
		static const int count(int i=0){return i==0?c0:sublist::count();};
		int2 size()const{return int2(count(0),count(1));};
	public:
		titerator getiterator()const{
			return tindex(int2(count(0),count(1)));
		};
